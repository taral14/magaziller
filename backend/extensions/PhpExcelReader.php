...
class OLERead {
	...
	function read($sFileName){
		...
		$this->numBigBlockDepotBlocks = GetInt4d($this->data, self::NUM_BIG_BLOCK_DEPOT_BLOCKS_POS);
		$this->sbdStartBlock = GetInt4d($this->data, self::SMALL_BLOCK_DEPOT_BLOCK_POS);
		$this->rootStartBlock = GetInt4d($this->data, self::ROOT_START_BLOCK_POS);
		$this->extensionBlock = GetInt4d($this->data, self::EXTENSION_BLOCK_POS);
		$this->numExtensionBlocks = GetInt4d($this->data, self::NUM_EXTENSION_BLOCK_POS);
	...
	function __readPropertySets(){
		...
		while ($offset < strlen($this->entry)) {
			...
			$startBlock = GetInt4d($d, self::START_BLOCK_POS);
			$size = GetInt4d($d, self::SIZE_POS);
			$name = '';
...
class PhpExcelReader {
	...
	private $colindexes = array();
	private $standardColWidth = 0;
	private $defaultColWidth = 0;
	...
	var $_defaultFormat = self::DEF_NUM_FORMAT;
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
	...
	function _parsesheet($spos) {
		...
		while($cont) {
			...
			if ($lowcode == self::TYPE_EOF) break;
